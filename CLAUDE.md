...
Pricing lives in `packages/db/src/pricing.ts`. It returns a `PriceBreakdown` with full GST at 18% (9% CGST + 9% SGST). STUDIO rooms with ≥28 nights automatically use monthly rates even if `bookingType` is `DAILY` — check the `isMonthly` flag on the returned breakdown.
...
- `guest-portal` — GUEST only
- `front-office` — FRONT_OFFICE or higher
...
- `ok()`, `created()`, `notFound()`, `badRequest()`, `serverError()` from `@the-rooms/api/response`
