...
interface Guest { id: string; name: string; phone: string; email?: string; companyName?: string; stayCount: number; loyaltyTier: string; bookings: Array<{ id: string; checkIn: string; checkOut: string; status: string; room: { roomNumber: string; type: string } }> }
...
export default function GuestsPage() {
  ...
  const searchGuests = useCallback(async (query: string) => {
    if (query.length < 2) { setGuests([]); setSearched(false); return; }
    setLoading(true); setSearched(true);
    try { const res = await fetch(`/api/guests/search?q=${encodeURIComponent(query)}`); if (res.ok) { const data = await res.json(); setGuests(data.guests ?? []); } }
    catch { setGuests([]); }
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && searched && guests.length === 0 && <div className="rounded-xl border bg-white py-12 text-center"><User className="h-12 w-12 text-gray-300 mx-auto mb-4" /><p className="text-gray-500">No guests found</p></div>}
