...
interface Booking { id: string; bookingNumber: string; status: string; paymentStatus: string; checkIn: string; checkOut: string; guest: { id: string; name: string; phone: string; email?: string }; room: { roomNumber: string; type: string }; signatureUrl?: string }
const STEPS = [{ id: 1, name: "Verify", icon: Calendar }, { id: 2, name: "Documents", icon: Camera }, { id: 3, name: "Signature", icon: PenTool }, { id: 4, name: "Complete", icon: CheckCircle }];
...
export default function CheckInPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 print:m-0 print:p-0 print:max-w-none">
      ...
      <div className="flex items-center justify-between overflow-x-auto pb-2 print:hidden">
        {STEPS.map((s, i) => (
          <div key={s.id} className="flex items-center">
            <button disabled={step < s.id} className={cn("flex items-center gap-2 rounded-full px-3 py-2 text-xs font-medium whitespace-nowrap", step === s.id ? "bg-[#E17055] text-white" : step > s.id ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-400")}>
              <s.icon className="h-4 w-4" />{s.name}
            </button>
            {i < STEPS.length - 1 && <div className={cn("h-0.5 w-8 mx-1", step > s.id ? "bg-green-400" : "bg-gray-200")} />}
          </div>
        ))}
      </div>
      ...
      {step === 1 && booking && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 space-y-6 print:hidden">
          <h3 className="text-lg font-semibold text-gray-900">Verify Booking</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div><p className="text-sm text-gray-500">Guest</p><p className="font-medium text-lg">{booking.guest.name}</p></div>
              <div><p className="text-sm text-gray-500">Phone</p><p className="font-medium">{booking.guest.phone}</p></div>
            </div>
            <div className="space-y-4">
              <div><p className="text-sm text-gray-500">Check-in</p><p className="font-medium">{formatDate(booking.checkIn, "long")}</p></div>
              <div><p className="text-sm text-gray-500">Check-out</p><p className="font-medium">{formatDate(booking.checkOut, "long")}</p></div>
              <div><p className="text-sm text-gray-500">Room</p><p className="font-medium">{booking.room.roomNumber} ({booking.room.type})</p></div>
            </div>
          </div>
          <button onClick={() => setStep(2)} className="w-full rounded-lg bg-[#E17055] py-3 text-white hover:bg-[#D35B3F]">Confirm & Continue</button>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 space-y-6 print:hidden">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {["AADHAAR", "PASSPORT", "VOTER_ID", "DRIVING_LICENSE"].map((type) => (
                <button key={type} onClick={() => setDocumentType(type)} className={cn("rounded-lg border-2 py-3 text-sm font-medium", documentType === type ? "border-[#E17055] bg-[#E17055]/5 text-[#E17055]" : "border-gray-200 text-gray-600")}>{type.replace("_", " ")}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(1)} className="flex-1 border border-gray-300 py-3 text-gray-700 hover:bg-gray-50">Back</button>
            <button onClick={() => setStep(3)} disabled={!frontImage || !backImage} className="flex-1 bg-[#E17055] py-3 text-white hover:bg-[#D35B3F] disabled:opacity-50">Continue</button>
          </div>
        </div>
      )}
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 space-y-6 print:hidden">
          ...
          {signatureUrl && (
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
              ...
              <span className="text-sm text-green-700 font-medium">Signature captured successfully</span>
            </div>
          )}

          <div className="flex gap-3 pt-4">
            <button onClick={() => setStep(2)} className="flex-1 border border-gray-300 py-3 text-gray-700 hover:bg-gray-50">Back</button>
            <button onClick={handleCheckIn} disabled={!signatureUrl || isSubmitting} className="flex-1 bg-[#E17055] py-3 text-white hover:bg-[#D35B3F] disabled:opacity-50 flex items-center justify-center gap-2">
              {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : null} Complete Check-In
            </button>
          </div>
        </div>
      )}
        ...
        <>
          <div className="rounded-xl border border-gray-200 bg-white p-8 text-center space-y-6 print:hidden">
            ...
            <div className="flex gap-3">
              ...
              <Link href="/rooms/board" className="flex-1 flex justify-center bg-[#E17055] py-3 text-white hover:bg-[#D35B3F] font-medium rounded-lg">
                View Room Board
              </Link>
            </div>
          </div>
          ...
          <div className="hidden print:block w-[80mm] mx-auto text-black bg-white p-4 font-mono text-sm">
            ...
            <div className="space-y-2 mb-6">
              ...
              <div className="flex justify-between"><span>Guest:</span><span className="font-bold">{booking.guest.name}</span></div>
              <div className="flex justify-between"><span>Check-Out:</span><span className="font-bold">{formatDate(booking.checkOut, "short")}</span></div>
            </div>

            <div className="border-t-2 border-black pt-4 pb-12 text-center text-xs">
              <p>Welcome to The Rooms!</p>
