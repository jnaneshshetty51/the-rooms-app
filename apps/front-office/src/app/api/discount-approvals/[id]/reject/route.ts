// apps/front-office/src/app/api/discount-approvals/[id]/reject/route.ts
// Reject Discount Approval Request
...
import { z } from 'zod';
import { rejectDiscountRequest, getDiscountRequestById } from '@the-rooms/db/queries/discountApprovalQueries';
...
const rejectDiscountSchema = z.object({
...
export async function POST(
    ...
    try {
        ...
        const parsed = rejectDiscountSchema.safeParse(body);
        ...
        const result = await rejectDiscountRequest(id, {
            rejectedById: userId!,
            rejectionReason,
        });
        ...
        console.error('[DISCOUNT_APPROVAL_REJECT]', error);
