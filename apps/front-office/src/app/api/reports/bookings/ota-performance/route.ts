...
import { getOTAPerformance } from "@the-rooms/db/queries/bookingSourceQueries";

// GET /api/reports/bookings/ota-performance - Get OTA metrics
export async function GET(request: NextRequest) {
    try {
        ...
        const performance = await getOTAPerformance(
            propertyId,
            new Date(startDate),
            new Date(endDate)
        );
        ...
        console.error("Error fetching OTA performance:", error);
        return serverError();
    }
}
