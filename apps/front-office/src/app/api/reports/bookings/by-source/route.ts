...
import { getBookingSourceAnalytics } from "@the-rooms/db/queries/bookingSourceQueries";

// GET /api/reports/bookings/by-source - Get bookings by source
export async function GET(request: NextRequest) {
    try {
        ...
        const analytics = await getBookingSourceAnalytics(
            propertyId,
            new Date(startDate),
            new Date(endDate)
        );
