...
import { Loader2, Sparkles, AlertCircle, Clock, Trash2, User, CheckCircle } from "lucide-react";
...
export default function HousekeepingDashboard() {
  ...
  const [filter, setFilter] = useState<"ALL" | "DIRTY" | "CLEANING" | "CLEAN">("DIRTY");
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      <header className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div>
          ...
          <p className="text-sm text-gray-500">Room Cleaning Management</p>
        </div>
        <button
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <Trash2
                className={cn(
                  "w-5 h-5",
                  filter === "DIRTY" ? "text-red-600" : "text-gray-400"
                )}
              />
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <Clock
                className={cn(
                  "w-5 h-5",
                  filter === "CLEANING" ? "text-blue-600" : "text-gray-400"
                )}
              />
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <CheckCircle
                className={cn(
                  "w-5 h-5",
                  filter === "CLEAN" ? "text-green-600" : "text-gray-400"
                )}
              />
              <span className="font-semibold text-gray-900">Clean</span>
            </div>
          ...
          <div className="space-y-3">
            {filteredRooms.map((room) => (
              <Link
                ...
                key={room.id}
                className="block bg-white p-4 rounded-xl border shadow-sm active:scale-[0.98] transition-transform"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {room.cleanedBy && room.lastCleanedAt && (
                        <p className="text-xs text-gray-400 mt-1">
                          Last cleaned by {room.cleanedBy.name} on{" "}
