...
import type { LoyaltyTier } from "@/components/loyalty/LoyaltyBadge";
...
type Stats = {
  ...
  loyaltyProgress?: number; // percentage to next tier
};
...
export default function GuestDashboardPage() {
  ...
  // Demo loyalty data (in real app, this would come from API/user session)
  const loyaltyTier: LoyaltyTier = stats?.loyaltyTier || 'BRONZE';
  const loyaltyProgress = stats?.loyaltyProgress || 45;
  const nextTierName = loyaltyTier === 'BRONZE' ? 'Silver' : loyaltyTier === 'SILVER' ? 'Gold' : undefined;
  ...
  return (
    <div className="space-y-6">
      {/* Page header with greeting and loyalty */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#2D3436]">
            {getGreeting()}{nextBooking ? `, ${nextBooking.guest.name.split(' ')[0]}` : '!'}
          </h1>
        ...
        <div className="shrink-0">
          <LoyaltyBadge 
            tier={loyaltyTier} 
            size="md"
            nextTier={nextTierName ? { name: nextTierName, progress: loyaltyProgress } : undefined}
          />
        </div>
      </div>
